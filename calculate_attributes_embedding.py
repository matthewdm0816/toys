...
parser.add_argument("--lbl2idx_path", type=str, default="~/data/vqav2/lbl2idx_separate_top3k.pkl")
parser.add_argument("--answer_bs", type=int, default=16)
parser.add_argument("--model_type", type=str, default="RN50x16")
parser.add_argument("--tokenized_dataset_path", type=str, default="/home/yangliu/data/vqav2/processed_tokenized_separate_top3k_new")
parser.add_argument("--save_path", type=str, default="/home/yangliu/data/bert-vqa")
...
vinvl_path = opt.tokenized_dataset_path
...
ic(idx2lbl_union[1456])
ic(idx2lbl_union[1777], lbl2idx_union[idx2lbl_union[1777]])
# ic(lbl2idx_union["UNK"], idx2lbl_union[lbl2idx_union["UNK"]])
ic(lbl2idx_union["baseline"])


# In[41]:
opt.postfix = "_" + opt.postfix if opt.postfix != "" else opt.postfix

# save merged embeddings and pointers
midfix = opt.model_type.replace("/", "_")
with open(os.path.expanduser(f"{opt.save_path}/a2v_{midfix}{opt.postfix}.pt"), "wb") as f:
    torch.save(embedded_answers_base, f)
with open(os.path.expanduser(f"{opt.save_path}/a2v_{midfix}{opt.postfix}_union.pt"), "wb") as f:
    torch.save(embedded_union, f)
with open(os.path.expanduser(f"{opt.save_path}/lbl2idx_{midfix}{opt.postfix}_union.pkl"), "wb") as f:
...
# ic(idx2lbl_union[1777], lbl2idx_union["UNK"])
